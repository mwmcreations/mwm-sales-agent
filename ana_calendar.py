...
def _parse_event_details(text):
    ...
    if title_match:
        ...
        if title_match:
            ...
            # Try "add X to the/my calendar" pattern first (common phrasing)
            title_match = re.search(
                r"(?:add|put)\s+(.+?)\s+(?:to\s+(?:the|my|our)\s+calendar|on\s+(?:the|my|our)\s+calendar)",
                text, re.IGNORECASE,
            )
            if title_match:
    ...
    if time_match:
        ...
        if ampm == "h":
            pass  # 24-hour format (e.g., "14h"), hour is already correct
